...
from kilimandjaro.source import get_category_scheme_from_ddi, get_snomed_terms, snomed_random_query, snomed_tension_query
import pathlib
import pprint
import chromadb
...
class Kilimandjaro(App):
    ...
    async def run_button_clicked(self) -> None:
        if self.state.db_setup:
            questionnaire_cs = await get_category_scheme_from_ddi(pathlib.Path.cwd() / "dat" / "constances-questionnaire.xml")

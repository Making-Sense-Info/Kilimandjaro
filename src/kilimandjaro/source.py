...
snomed_random_query = SPARQLQuery("Aléa, les 500 premiers", """
PREFIX dc: <http://purl.org/dc/elements/1.1/>
...
async def get_category_scheme_from_ddi(source_file_path:Path) -> list[str]:
    with open(source_file_path) as sf:
        tree = et.parse(sf)
        root = tree.getroot()
        category_schemes = root.findall(".//l:CategoryScheme[r:ID='CategoryScheme-lzmgjrzf']", namespaces=namespaces)
